...
class CornersProblem(search.SearchProblem):
  ...
  def isGoalState(self, state):
	...
	#in order for goal state to be found all four corners of a maze must be visited
	# so when visited corners length reaches 4 goal state is found
	...
	#successor method same as given one with slight changes to successors.append
		dx, dy = Actions.directionToVector(action)
		nextx, nexty = int(x + dx), int(y + dy)
		hitsWall = self.walls[nextx][nexty]
		if not hitsWall:
			next_state = (nextx, nexty)
			if next_node in corners:
...
def cornersHeuristic(state, problem):
	...
	#checking if all corners have been visited and if not add to non visited list
	for state in corners:
		if not state in visited:
			not_visited.append(state)
	
	#calculate distance from first starting point to next corner
	#summation of distance from each corner will return heuristic
	#remove corner as it is read
	while not not_visited.isEmpty():
		for corner in not_visited:
			distance = min(self.manhattanHeuristic(current_state, corner))
			total += distance
