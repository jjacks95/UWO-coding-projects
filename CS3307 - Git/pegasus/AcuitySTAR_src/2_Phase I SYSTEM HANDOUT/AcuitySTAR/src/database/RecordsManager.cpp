...
std::vector<BasicRecord*> RecordsManager::findRecordsInRange(int startYear, int endYear) {
    std::vector<BasicRecord*> returnList;
    std::vector<BasicRecord*> tempRecord;
    ...
    for (Manager::iterator itr = startPoint; itr != endPoint; itr++) {
        //if(itr->second.size() < this->headers)

        BasicRecord* tempRec = itr->second;
        if(tempRec->size() == this->headers.size() - 1)
        {
            tempRec->push_back("");
            returnList.push_back(tempRec);
        }
        else
        {
            returnList.push_back(itr->second);
...
int RecordsManager::getNumberOfHeaders(){
    return headers.size();
}
...
std::string RecordsManager::analyze(StringTree sortedTree, const std::vector<int> &sortFields, const std::vector<int> &accs, std::vector<double> &accsTotals,
    ...
    for (StringTree::iterator sortField = sortedTree.begin(); sortField != sortedTree.end(); sortField = uniqueValue.second) {
        ...
        for (StringTree::iterator itr = uniqueValue.first; itr != uniqueValue.second; itr++) {
            ...
            if ( (int) sortFields.size() - 1 > depth) {  // &&  ((int)sortFields.size() > depth + 1) ) {

                try
                {
                    newSorted.emplace(itr->second->at(sortFields[depth + 1]), itr->second);
//                    if(sortFields[depth + 1] == countCol  )
...
//                        newSorted.emplace(itr->second->at(sortFields[depth + 1]), itr->second);
                ...
                {
                    QTextStream(stdout) << "\nFailed at: newSorted.emplace. Mesage: " << ex.what()
                                        << " \n\t sortFields[depth + 1]: " << sortFields[depth + 1]
                                        << " \n\t depth: " << depth
                                        << " \t sortField.size(): " << sortFields.size()  << " \n Reason: Last Header is blank, please populate";
                }
           //   newSorted.emplace(itr->second->at(sortFields[depth + 1]), itr->second); // <------- This is the original
            } else {
